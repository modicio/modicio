<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/Fragment/Model">
        ...
        <xsl:for-each select="Node">
            ...
            <xsl:for-each select="AssociationRelation">
                ...
                <xsl:text>(</xsl:text><xsl:value-of select="replace(../@uri, 'modicio:', '')" /><xsl:text>, </xsl:text><xsl:value-of select="replace(@target, 'modicio:', '')"/><xsl:text>) .. I</xsl:text><xsl:value-of select="../@name"/><xsl:value-of select="@name"/><xsl:text>&#10;</xsl:text>
                <xsl:text>Interface I</xsl:text><xsl:value-of select="../@name"/><xsl:value-of select="@name"/><xsl:text>{}</xsl:text>
            </xsl:for-each>
